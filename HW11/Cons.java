            ...
            throws java.io.FileNotFoundException
    {
        ...
        final Mapper map2 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
                ...
                if(!value.equals("")) {             // if the value is a character
                    value = value.toLowerCase();    // set the value to be lower case, emulates case-insensitivity
                    for(String i = value; i.length() != 0; i = i.substring(1)) {    // walk through each character in the value
                        char c = i.charAt(0);
                        if(Character.isLetter(c)) {                                 // if the character is a letter
                            mr.collect_map(Character.toString(c), list("1"));       // map it to the value of 1
                        }
                    }
                }
            } };
        ...
        final Reducer red2 = new Reducer() {
            public void reduce(String key, Cons value, MapReduce mr) {
                ...
                int sum = 0;
                for ( Cons lst = value; lst != null; lst = rest(lst) )
                    sum += Integer.decode((String) first((Cons)             // adds up the number of occurrences of each character
                            first(lst)));
                mr.collect_reduce(key, new Integer(sum)); } };
        ...
        final Mapper map3 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
                ...
                if(!value.equals("")) {                     // uses replace() method to get rid of all punctuation
                    value = value.replace('.', ' ');        // only strings and spaces
                    value = value.replace('?', ' ');
                    value = value.replace('!', ' ');
                    value = value.replace(',', ' ');
                    value = value.replace('\'', ' ');
                    ...
                    value = value.replace('\"', ' ');
                    value = value.replace('-', ' ');
                    value = value.toLowerCase();            // makes everything lower case to emulate case-insensitivity
                    StringTokenizer t = new StringTokenizer(value);     // breaks value into tokens that can be enumerated
                    while(t.hasMoreTokens()) {
                        mr.collect_map(value = t.nextToken(), list("1"));
                    }
                }
            } };
        ...
        final Mapper map4 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
                ...
                while(value.contains(grepkey)) {                        // check the input for the desired word
                    mr.collect_map(grepkey, list(key));                 // add the line number (key)
                    value = value.substring(1 + value.indexOf(grepkey));  // continue on the next word
                }
            } };
        ...
        final Reducer red4 = new Reducer() {
            public void reduce(String key, Cons value, MapReduce mr) {
                ...
                Cons lines = null;
                for(Cons lst = value; lst != null; lst = rest(lst)) {       // add the lines where the word is on to the list
                    lines = append(lines, list(Integer.decode((String) first((Cons) first(lst)))));
                }
                mr.collect_reduce(key, lines);
            } };
        ...
        final Mapper map5 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
                ...
                if(!value.equals("")) {                     // uses replace() method to get rid of all punctuation
                    ...
                    StringTokenizer t = new StringTokenizer(value);     // breaks value into tokens that can be enumerated
                    while(t.hasMoreTokens()) {
                        mr.collect_map(value = t.nextToken(), list(key));
                    }
                }
            } };
        ...
        final Reducer red5 = new Reducer() {
            public void reduce(String key, Cons value, MapReduce mr) {
                ...
                Cons lines = null;
                for(Cons lst = value; lst != null; lst = rest(lst)) {       // lists the line of every occurrence of the word
                    lines = append(lines, list(Integer.decode((String) first((Cons) first(lst)))));
                }
                mr.collect_reduce(key, lines);
            } };
        ...
        final Mapper map6 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
                ...
                String movieNum = value.substring(0, value.indexOf(','));
                String valueb = value.substring(value.indexOf(',') + 1);
                String rating = valueb.substring(valueb.indexOf(',') + 1, valueb.indexOf(',') + 2);
                mr.collect_map(movieNum, list(rating));
            } };
        ...
        final Reducer red6 = new Reducer() {
            public void reduce(String key, Cons value, MapReduce mr) {
                ...
                for(Cons lst = value; lst != null; lst = rest(lst)) {
                    ...
                    sum += Integer.decode((String) first((Cons) first(lst)));   // get the total rating
                }
                mr.collect_reduce(key, list(sum/count, (int) count));           // return the average and the number of ratings
            } };
